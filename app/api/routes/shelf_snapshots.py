...
async def upload_shelf_snapshot(
    ...
    return SessionSnapshotPublic(
        ...
        image_url=public_object_url(object_name, settings.S3_SHELF_BUCKET),
        content_type=file.content_type,
        size=len(data),
    )
...
def list_shelf_snapshots(session_id: str) -> SessionSnapshotListPublic:
    ensure_bucket_exists(settings.S3_SHELF_BUCKET)
    client = get_object_storage()
    prefix = build_object_name(session_id, "")
    snapshots: list[SessionSnapshotPublic] = []

    for obj in client.list_objects(settings.S3_SHELF_BUCKET, prefix=prefix):
        ...
        stat = client.head_object(settings.S3_SHELF_BUCKET, obj.object_name)
        ...
        snapshots.append(
            SessionSnapshotPublic(
                ...
                image_url=public_object_url(obj.object_name, settings.S3_SHELF_BUCKET),

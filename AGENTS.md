...
Read `../AGENTS.md` first. Inspect Git with `git -C self-checkout-ml`; never edit directly on `main` or `master`, combine repositories in one commit, or commit `.env`, `.venv`, caches, datasets, snapshots, trained model artifacts, or S3 credentials. Destructive dataset/reset utilities require explicit user approval and must never be part of routine validation.
...
for captures, labeled uploads, training data, datasets, and model artifacts.
It must support custom endpoints, TLS verification, path-style addressing,
timeouts, and retries without provider-specific code. Coordinate storage,
training, model-registry, and inference contract changes with backend and infra
and validate the current stack on dev.
